...
const provider = new PackageJsonFileProvider({ required: 2, log: createTestLog() });
provider.self = {
...
test('PackageJsonFileProvider - getFilePaths', async t => {
  const paths = await provider.getFilePaths();

  t.deepEqual(paths, ['package.json']);
});

test('PackageJsonFileProvider - package.json', async t => {
  const reviews = await provider
    .getReviewsFromFile('owner', 'repo', 'sha', PACKAGE_JSON_FILE);
...
test('PackageJsonFileProvider - package.json no maintainers', async t => {
  const reviews = await provider
    .getReviewsFromFile('owner', 'repo', 'sha', '{}');
