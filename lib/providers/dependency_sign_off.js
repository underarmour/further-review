...
function packageJsonDiff(base, head) {
  ...
  const baseWithoutDependencies = Object.assign({}, base, { version: null });
  const headWithoutDependencies = Object.assign({}, head, { version: null });
  dependencyKeys.forEach(k => {
    delete baseWithoutDependencies[k];
    delete headWithoutDependencies[k];
  });

  dependencyDiff.version = base.version !== head.version;
...
class DependencySignOffProvider extends GlobSignOffProvider {
  ...
  renderSignOffDescription(so) {
    ...
    if (version) {
      description.push('Version changed');
    }
  ...
  async checkSignOff(so) {
    ...
    const { added, removed, changed, version, other } = so.packageJson;
