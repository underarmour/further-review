    ...
    return this.initProviders()
      ...
      .each(r => {
        ...
        if (originalLogins.length !== r.logins.length) {
          ...
          const invalidLogins = originalLogins.filter(v => r.logins.indexOf(v) < 0);
      ...
      return getMentions({ github: this.github, owner, repo, number })
        ...
        .then(reviewsMd => {
          if (reviewsMd.length > 0) {
            // get existing comment count, and stop at hard cutoff
            return this.github.getCurrentUser()
              .get('login')
              .then(login => this.github.getIssueComments({
                owner, repo, number,
              })
              .filter(c => c.user.login === login))
              .then(comments => {
                if (comments.length < this.config('max_mention_comments')) {
                  const body = `## Further Review Needed\n\n${reviewsMd.join('')}`;

                  return this.github.createComment({ owner, repo, number, body });
                }

                this.log.warn('No further comments allowed by configuration.');
