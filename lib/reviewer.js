...
class Reviewer {
  ...
  processReviews({ owner, repo, number }) {
    this.log.info(`Processing ${owner}/${repo} PR #${number}`);

    return this.github.getPullRequest({ owner, repo, number })
      .then(({ head: { sha }, user: { login: proposer } }) => {
        return { owner, repo, number, sha, proposer };
      })
      .tap(pr => this.updateStatus(Object.assign(
        {},
        pr,
        { state: 'pending', description: 'Processing PR' }
      )))
      .then(({ sha, proposer }) => {
        return getPullRequestFiles({ github: this.github, owner, repo, number })
          .then(files => {
            return {
