...
class Reviewer {
  ...
  renderComment(results, state) {
    ...
    const headingMap = results.reduce((map, { title, name, comment }) => {
      if (comment) {
        const heading = title || name;
        const comments = map.get(heading) || [];
        comments.push(comment);
        map.set(heading, comments);
      }
      return map;
    }, new Map());

    for (const [heading, comments] of headingMap) {
      md.push(`### ${heading}`, ...comments);
    }
