...
test('Reviewer.review - already commented', async t => {
  class TestProvider extends BaseProvider {
    async review() {
      return {
        success: false,
  ...
  if (t.context.github.createComment.calledOnce) {
    // Shouldn't get here because the comments should match
    // This assertion is just to help debugging.
    t.is(t.context.github.createComment.args[0][0].body, expectedBody);
    t.fail('createComment should not be called');
  }
